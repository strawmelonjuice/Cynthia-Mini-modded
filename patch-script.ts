...
async function applyPatches() {
  ...
  await $`rm -rf ${cloneDir}/test`;

  if (existsSync("./test")) {
    await $`mv ./test ${cloneDir}/test`;
  } else {
    await $`git clone https://github.com/strawmelonjuice/strawmelonjuice-site ${cloneDir}/test`;
  }
  // Re-make output directory
  await $`rm -rf ${cloneDir}/test/out`;
  await $`mkdir ${cloneDir}/test/out`;
